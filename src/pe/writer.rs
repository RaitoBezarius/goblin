...
impl<'pe, 'b> PEWriter<'pe, 'b> {
    ...
    fn layout_data_directories_contents(
            ...
            .enumerate()
        {
            ...
            if let Some(dd) = dir {
                dd.offset = Some(self.file_size as usize);
    ...
    fn write_headers(&mut self, buf: &mut Vec<u8>) -> error::Result<usize> {
        let offset = &mut 0;
        // 1. write the header
        trace!("writing this header: {:#?}", self.pe.header);
    ...
    pub fn write_into(&mut self) -> error::Result<Vec<u8>> {
        ...
        debug_assert!(
            self.pe.header.optional_header.map(|opt_header| {
                ...
                // table ordering.
                let dd_end = opt_header
                    .data_directories
                    .get_debug_table()
                    .and_then(|debug_table| 
                        debug_table.offset
                        .map(|offset| offset as u32 + debug_table.size)
                    );
                let cert_attr_end = opt_header.data_directories.get_certificate_table()
                    .and_then(|cert_table|
                        enumerate_certificates(&buffer, cert_table.virtual_address,
                            cert_table.size)
                        .ok()
                    ).and_then(|certificates| {
                        certificates.last().map(|(last_offset, cert)| *last_offset as u32 + cert.length)
                    });
